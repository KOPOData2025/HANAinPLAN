...
import com.hanainplan.shinhan.account.dto.TransactionRequestDto;
...
public class AccountService {
    ...
    public String processDeposit(String accountNumber, BigDecimal amount, String description) throws Exception {
        log.info("신한은행 계좌 입금 처리 시작 - 계좌번호: {}, 금액: {}원", accountNumber, amount);
        ...
        log.info("신한은행 계좌 입금 처리 완료 - 거래ID: {}, 계좌번호: {}, 새 잔액: {}원", 
                transactionId, accountNumber, newBalance);
    ...
    private String saveDepositTransaction(Account account, BigDecimal amount, BigDecimal balanceAfter, String description) {
        try {
            String transactionId = "SH-DP-" + System.currentTimeMillis() + "-" + 
                    String.format("%04d", (int)(Math.random() * 10000));
            ...
            log.info("신한은행 입금 거래내역 저장 완료 - 거래ID: {}, 계좌번호: {}, 금액: {}원", 
                    transactionId, account.getAccountNumber(), amount);

            return transactionId;

        } catch (Exception e) {
            log.error("신한은행 입금 거래내역 저장 실패 - 계좌번호: {}, 오류: {}", account.getAccountNumber(), e.getMessage());
            throw new RuntimeException("거래내역 저장 실패: " + e.getMessage());
        }
    }

    public String createTransaction(TransactionRequestDto request) {
        log.info("신한은행 거래내역 생성 요청 - 계좌번호: {}, 거래유형: {}, 금액: {}원", 
        ...
        String transactionId = "SH-TX-" + System.currentTimeMillis() + "-" + 
                String.format("%04d", (int)(Math.random() * 10000));
        ...
        Transaction transaction = Transaction.builder()
                ...
                .branchName(request.getBranchName() != null ? request.getBranchName() : "신한은행 본점")
                .account(account)
                .build();

        transactionRepository.save(transaction);

        log.info("신한은행 거래내역 생성 완료 - 거래ID: {}, 계좌번호: {}, 거래유형: {}, 금액: {}원", 
