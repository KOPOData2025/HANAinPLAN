package com.hanainplan.domain.fund.init;
...
public class FundDataLoader implements CommandLineRunner {
    ...
    private void createMiraeAssetPlan20() {
        FundMaster master = fundMasterRepository.findById("513061")
                .orElseGet(() -> fundMasterRepository.save(FundMaster.builder()
                        .fundCd("513061")
                        .fundName("미래에셋퇴직플랜20증권자투자신탁1호(채권혼합)")
                        .fundGb(2)
                        .assetType("채권혼합")
                        .riskGrade(String.valueOf(random.nextInt(5) + 1))
                        .currency("KRW")
                        .isActive(true)
                        .build()));
