...
import { getIrpPortfolioSummary, type IrpPortfolioSummary } from '../../api/portfolioApi';
...
export default function IrpRebalancing({ customerId, irpAccountNumber: _irpAccountNumber }: IrpRebalancingProps) {
  ...
  const [currentPortfolio, setCurrentPortfolio] = useState<IrpPortfolioSummary | null>(null);
  ...
  useEffect(() => {
    const fetchCurrentPortfolio = async () => {
      try {
        const portfolioSummary = await getIrpPortfolioSummary(customerId);
        setCurrentPortfolio(portfolioSummary);
      } catch (error) {
  ...
  // 현재 포트폴리오 차트 데이터 변환
  const getCurrentPortfolioChartData = () => {
    if (!currentPortfolio) {
      return [
        { name: '현금', value: 100, amount: 0, color: COLORS.cash },
        { name: '예금', value: 0, amount: 0, color: COLORS.deposit },
        { name: '펀드', value: 0, amount: 0, color: COLORS.fund }
      ];
    }

    const totalValue = currentPortfolio.totalValue || 0;
    const cashAmount = currentPortfolio.cash?.amount || 0;
    const depositAmount = currentPortfolio.deposit?.amount || 0;
    const fundAmount = currentPortfolio.fund?.amount || 0;

    return [
      { 
        name: '현금', 
        value: totalValue > 0 ? (cashAmount / totalValue) * 100 : 0, 
        amount: cashAmount, 
        color: COLORS.cash 
      },
      { 
        name: '예금', 
        value: totalValue > 0 ? (depositAmount / totalValue) * 100 : 0, 
        amount: depositAmount, 
        color: COLORS.deposit 
      },
      { 
        name: '펀드', 
        value: totalValue > 0 ? (fundAmount / totalValue) * 100 : 0, 
        amount: fundAmount, 
        color: COLORS.fund 
      }
    ];
      ...
      {currentPortfolio && (
        <div className="mb-8">
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-6 border border-gray-200">
            <h3 className="text-lg font-hana-bold text-gray-900 mb-6">현재 자산 비중</h3>
            
            {/* 현재 포트폴리오 비중 - 원형 차트와 항목별 비중 */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-6">
              ...
              <div className="w-full h-64 flex items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={getCurrentPortfolioChartData()}
                      cx="50%"
                      cy="50%"
                      innerRadius={60}
                      outerRadius={100}
                      ...
                      dataKey="value"
                    >
                      {getCurrentPortfolioChartData().map((entry, index) => (
                    ...
                    <Tooltip 
                      formatter={(value: number, entry: any) => [
                        `${value.toFixed(1)}%`, 
                        `${entry.payload?.name || 'Unknown'} (${entry.payload?.amount?.toLocaleString() || '0'}원)`
                      ]}
                      contentStyle={{
              ...
              <div className="space-y-4">
                {getCurrentPortfolioChartData().map((item, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-white rounded-lg border">
                    <div className="flex items-center">
                      ...
                      ></div>
                      <span className="font-hana-medium text-gray-700">{item.name}</span>
                    </div>
                    <div className="text-right">
                      <div className="font-hana-bold text-gray-900">{item.value.toFixed(1)}%</div>
                      <div className="text-sm text-gray-500">{item.amount.toLocaleString()}원</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="text-center pt-4 border-t border-gray-300">
              <div className="text-lg font-hana-medium text-gray-800">
                총 IRP 자산: <span className="font-hana-bold text-gray-900">{(currentPortfolio?.totalValue || 0).toLocaleString()}원</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="mb-4 p-3 bg-blue-100 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-lg font-hana-bold text-blue-900">{recommendation.riskProfileDescription}</div>
                </div>
              </div>
            </div>
                ...
                <ResponsiveContainer width="100%" height={200} key={`current-${simulation?.jobId || 'empty'}`}>
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {getPortfolioChartData(simulation?.currentPortfolio).map((_entry, index) => (
                ...
                <ResponsiveContainer width="100%" height={200} key={`target-${simulation?.jobId || 'empty'}`}>
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {getPortfolioChartData(simulation?.targetPortfolio).map((_entry, index) => (
                        <Cell key={`cell-${index}`} fill={COLORS_ARRAY[index]} />
                      ))}
                    </Pie>
                    <Tooltip formatter={(value, name) => [`${value}%`, name]} />
                ...
                <ResponsiveContainer width="100%" height={200} key={`expected-${simulation?.jobId || 'empty'}`}>
                  <PieChart>
                    <Pie
                      ...
                      dataKey="value"
                    >
                      {getPortfolioChartData(simulation?.expectedPortfolio).map((_entry, index) => (
                        <Cell key={`cell-${index}`} fill={COLORS_ARRAY[index]} />
                      ))}
                    </Pie>
                    <Tooltip formatter={(value, name) => [`${value}%`, name]} />
