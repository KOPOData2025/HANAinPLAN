...
interface SimpleDepositPortfolioProps {
  irpAccountNumber: string;
}

function SimpleDepositPortfolio({ irpAccountNumber }: SimpleDepositPortfolioProps) {
  ...
  const fetchPortfolioData = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`/api/banking/portfolio/irp/${irpAccountNumber}/active`);
          ...
          <div className="space-y-4">
            {portfolios.map((portfolio) => (
              <div
                ...
                className="border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow w-full"
              >
                {/* 상품 헤더 */}
                ...
                <div className="space-y-3 mb-4">
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">투자 원금</span>
                    <span className="text-base font-hana-bold text-gray-900">{formatCurrency(portfolio.principalAmount)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">적용 금리</span>
                    <span className="text-base font-hana-bold text-hana-green">{portfolio.interestRate}%</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">예상 이자</span>
                    <span className="text-base font-hana-bold text-hana-green">+{formatCurrency(portfolio.expectedInterest)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-500">만기 예상액</span>
                    <span className="text-base font-hana-bold">{formatCurrency(portfolio.maturityAmount)}</span>
                  </div>
                </div>
...
export default SimpleDepositPortfolio;
