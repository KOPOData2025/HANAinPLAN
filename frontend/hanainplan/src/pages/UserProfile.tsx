...
function UserProfile() {
  ...
  // 사용자 정보 완전히 로드
  const loadCompleteUserInfo = async () => {
    if (!user?.userId) return;
    try {
      const userInfoResponse = await getUserInfo(user.userId);
      const { userBasicInfo, customerDetailInfo } = userInfoResponse;
      ...
      setUser({
        ...
        userType: userBasicInfo.userType as 'GENERAL' | 'COUNSELOR',
        gender: userBasicInfo.gender as 'M' | 'F',
        birthDate: userBasicInfo.birthDate,
        loginType: userBasicInfo.loginType,
        isPhoneVerified: userBasicInfo.isPhoneVerified,
        isActive: userBasicInfo.isActive,
        createdDate: userBasicInfo.createdDate,
        lastLoginDate: userBasicInfo.lastLoginDate,
  ...
  // 컴포넌트 마운트 시 사용자 정보와 리스크 프로파일 로드
  useEffect(() => {
    loadCompleteUserInfo();
    loadRiskProfile();
  }, [user?.userId]);
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900">
                    {user.lastLoginDate ? new Date(user.lastLoginDate).toLocaleDateString('ko-KR') : '-'}
                  </div>
                </div>
              </div>

              {/* 직업 정보 섹션 */}
              {user.customerDetailInfo?.jobInfo && (
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <h3 className="text-lg font-hana-medium text-gray-900 mb-4">직업 정보</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                      ...
                      <div className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900">
                        {user.customerDetailInfo.jobInfo.industryName}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900">
                        {user.customerDetailInfo.jobInfo.careerYears}년
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900">
                        {user.customerDetailInfo.jobInfo.assetLevel === 'FROM_10_TO_30' ? '1,000만원 ~ 3,000만원' : user.customerDetailInfo.jobInfo.assetLevel}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* 건강 정보 섹션 */}
              {user.customerDetailInfo?.healthInfo && (
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <h3 className="text-lg font-hana-medium text-gray-900 mb-4">건강 정보</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-hana-medium text-gray-700 mb-2">
                        주요 질병 여부
                      ...
                      <div className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900">
                        {user.customerDetailInfo.healthInfo.majorDisease ? '있음' : '없음'}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg text-gray-900">
                        {user.customerDetailInfo.healthInfo.longTermMedication ? '있음' : '없음'}
                      </div>
                    </div>
                  </div>

                  {user.customerDetailInfo.healthInfo.majorDisease && user.customerDetailInfo.healthInfo.diseaseDetails.length > 0 && (
                    <div className="mt-4">
                      <label className="block text-sm font-hana-medium text-gray-700 mb-2">
                        질병 상세 정보
                      </label>
                      <div className="space-y-3">
                        {user.customerDetailInfo.healthInfo.diseaseDetails.map((disease, index) => (
                          <div key={index} className="bg-red-50 border border-red-200 rounded-lg p-4">
                            <div className="flex justify-between items-start mb-2">
                              <h4 className="font-hana-medium text-red-800">{disease.diseaseName}</h4>
                              <span className="bg-red-100 text-red-800 px-2 py-1 rounded-full text-xs font-hana-medium">
                                {disease.severity === 'moderate' ? '중등도' : disease.severity}
                              </span>
                            </div>
                            <p className="text-sm text-red-700">{disease.description}</p>
                          </div>
                        ))}
                      </div>
                    </div>
